...
mist_1: # Preambulo mist_1
	...
	sw	$s0, 16($sp)	# 2 Almacenar registros guardados
	sw	$ra, 20($sp)	# 3 Guardar $ra
	sw	$fp, 24($sp)	# 4 Establecer $fp
...
loop_1: beqz $s0, end_1
	...
	# Retorno de mist_0
	lw	$t0, 28($sp)	# Restaurar $t0 con el valor de a
	lw	$t1, 32($sp)	# Restaurar $t1 con el valor de b
...
end_1: # Conclusion mist_1
	...
	lw	$s0, 16($sp)	# 2 Restaurar registros guardados
	lw	$ra, 20($sp)	
	lw 	$fp, 24($sp)
	addi	$sp, $sp, 40	# 3 Eliminar el marco
	jr	$ra		# 4 Regresar el control a la rutina invocador
